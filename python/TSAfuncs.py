    ...
    def ts_test_retest(self, **kwargs):
        ...
        if refinv is not None and refinv != 'logm':
            ...
            for reg in regvals:
                ...
                if refinv == 'logm':
                    refinv_mat = np.linalg.inv(np.mean(np.array([logm(x.fc + reg) for x in self.database]), axis = 0))
                else:
                    refinv_mat = None
                [x.tangent_space_projection(reg = reg, refinv = refinv_mat) for x in self.database]
                [x.tangent_space_projection(reg = reg, refinv = refinv_mat) for x in self.target ]
